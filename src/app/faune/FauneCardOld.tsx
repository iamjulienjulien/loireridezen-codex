...
/**
 * Anciennes variantes de la fiche faune.
 *
 * Conservées comme références graphiques et pour faciliter les comparaisons
 * avec la version actuellement utilisée dans FauneCard.
 */
export default function FauneCardOld({
    version = 1,
    ...props
}: FauneCardOldProps) {
    switch (version) {
...
type LegacyCardProps = Omit<FauneCardOldProps, "version">;

export function FauneCardV1({ d, open, onToggle }: LegacyCardProps) {
    return (
        <article
...
export function FauneCardV3({ d, open, onToggle }: LegacyCardProps) {
