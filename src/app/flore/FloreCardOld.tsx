...
export type FloreCardOldProps = {
    d: Flore;
    open: boolean;
    onToggle: () => void;
    /** Variante historique de la carte. */
    version?: 1 | 3;
};

type FloreCardVariantProps = Omit<FloreCardOldProps, "version">;

export default function FloreCardOld({
    version = 1,
    ...props
}: FloreCardOldProps) {
    switch (version) {
...
export function FloreCardV1({ d, open, onToggle }: FloreCardVariantProps) {
...
export function FloreCardV3({ d, open, onToggle }: FloreCardVariantProps) {
    const ind = INDIGENAT[d.statut.indigenat] ?? INDIGENAT.exotique;
    const prot = PROTECTION[d.statut.protection] ?? PROTECTION.aucune;
